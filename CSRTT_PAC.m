% This code will calculate for each session the PAC using following
% methods:
% 1. PLV across trials (the conventional way of calculating PAC
% and also the way we used in our paper, and what is used in calculating tPAC)
% 2. Coherence across trials
% 3. Amplitude correlation across trials (also with p value)

% AH 20210408: each session is taking forever to convolve, so I cut the LFP
% around the event time and concat to get convolution to save time, with 
% extra padding to reduce edge effect
...
level = '7'; % 1 or 2 digits are fine, for cluster, use 1 digit
skipRec = 1;
...
%for irec = 1:numel(fileInfo)     
    ...
    for iCond = 1:numConds
        ...
        if exist([rootAnalysisDir analysisType '_' alignHitName condName '_AmpCorr.png']) && skipRec == 1
        ...
        for iRegion = 1:numRegions        
            regionLFPcut{iRegion} = regionLFP{iRegion}(keepMask); % about 1/10 length now
        end
        numPadvec = sum(keepMask); % or numel(regionLFPcut{iRegion};
        %dvec = 1:cFS:numPadvec; % if use whole recording, downsample
        %here too
        % Pre-allocate NaN array
        for iRegion = 1:numRegions
            regionName = regionNames{iRegion};
            %convMat.(regionName) = nan(numFreqs,numFreqs,numPadvec);
            convMatEvent.(regionName) = nan(numel(evtTime),numel(numFreqs),numel(numFreqs),diff(tsamps)+1); %nTrial x nFreq x nFreq x nTimepoints
        end
        newEvtSamp = [round(-padtwin(1)*newFs):diff(padtwin)*newFs:numPadvec]; % newEvtTime[5,12,19...]s;

        % Convolve with wavelet
        for iRegion = 1:numRegions
            regionName = regionNames{iRegion};
            display(['Convolving ' condName ' ' regionName ' ' num2str(numFreqs) ' phase freq'])
            
            for fi = 1:numFreqs
                tmp = conv(regionLFPcut{iRegion},wavs{fi},'same'); % 1xtime complex
                ...
                for iev = 1:numel(evtTime) % for each trial
                    ...
                    convMatEvent.(regionName)(iev,fi,fi,:) = tmp(evSamp+tsamps(1):evSamp+tsamps(2)); % eg. 2-5s, 9-12s,...
                end
                for fj = fi+1:numFreqs
                    tmp1 = conv(regionLFPcut{iRegion},wavs{fj},'same');
                    tmp2 = conv(abs(tmp1),wavs{fi},'same');
                    %convMat.(regionName)(fi,fj,:) = tmp2(dvec);
                    %convMat(fi,fj,:) = tmp2;
                    
                    % Epoching data
                    for iev = 1:numel(evtTime) % for each trial
                        evSamp = newEvtSamp(iev);
                        convMatEvent.(regionName)(iev,fi,fj,:) = tmp2(evSamp+tsamps(1):evSamp+tsamps(2)); % eg. 2-5s, 9-12s,...
                    end
                end
            end
            angEvent.(regionName) = angle(convMatEvent.(regionName));
        end % end of iRegion
        
        
        %% compute PLV and coherence        
        for fi = 1:numFreqs % phase freq
            display(['Computing ' analysisType ' for ' num2str(fi) '/' num2str(numFreqs) ' phase freq: ' condName])
            for fj = fi+1:numFreqs % amp freq
                for iRegionX = 1:numRegions
                    ...
                    for iRegionY = 1:numRegions
                        ...
                        % PLV method (across trial, then mean over 3s)
                        angPhase = reshape(angEvent.(regionNameX)(:,fi,fi,:),nEvent,nSamp);
