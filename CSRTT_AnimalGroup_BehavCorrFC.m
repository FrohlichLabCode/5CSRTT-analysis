...
% V2 AH@20200404: 
% restricting behav to filter out level at beginning, so don't need to filter later
% add nAnimal flag, change file name, figure title
% use nPx, nSess
...
statBehavAll = is_load([GroupAnalysisDir 'statBehavAll_123A.mat'],'statBehavAll'); %3A: abcd levels, 4A: ab levels 
if do0179 == 1
...
% Prime empty table
nanTable = table(NaN(nSess,nFoi),'VariableNames',{'Value'});        
for iPair = 1:numRegionPairs
    regionPairName = region.PairNames{iPair};
    regionPair_Name = region.Pair_Names{iPair};
    regionName2 = split(regionPairName,'-');
    for iOpto = 1:numOptos
        ...
        statFCAll.(['Spec_' regionName2{2}]).(optoName) = unique(statBehavAccP7b(:,1:4));
        statFCAll.(['Spec_' regionName2{2}]).(optoName)(:,'Value') = nanTable; % for each freq
        end
        statFCAll.(['PLV_' regionPair_Name]).(optoName) = unique(statBehavAccP7b(:,1:4));
        statFCAll.(['PLV_' regionPair_Name]).(optoName)(:,'Value') = nanTable; % for each freq
    end
    for iOpto = 1:numOptos-1 % contrast
        optoName = optoNames{optoIDs(iOpto)};
        if iPair == 1 || iPair == 4
        statFCAll.(['Spec_' regionName2{1}]).([optoName '_Sham']) = unique(statBehavAccP7b(:,1:4));
        statFCAll.(['Spec_' regionName2{1}]).([optoName '_Sham'])(:,'Value') = nanTable; % for each freq
        statFCAll.(['Spec_' regionName2{2}]).([optoName '_Sham']) = unique(statBehavAccP7b(:,1:4));
        statFCAll.(['Spec_' regionName2{2}]).([optoName '_Sham'])(:,'Value') = nanTable; % for each freq
        end
        statFCAll.(['PLV_' regionPair_Name]).([optoName '_Sham']) = unique(statBehavAccP7b(:,1:4));
        statFCAll.(['PLV_' regionPair_Name]).([optoName '_Sham'])(:,'Value') = nanTable; % for each freq
        % for division
        statFCAll.(['PLV_' regionPair_Name]).([optoName '_ShamD']) = unique(statBehavAccP7b(:,1:4)); 
        ...
        for iAlign = 1:numAligns
            ...
            for iOpto = 1:numOptos
                ...
                if iPair == 1 || iPair == 4 % LPl-PPC, PFC-VC
                    ...
                    mat2 = AH_resample(pow2db(spec.avgYSpec),newFs,oldFs); 
                    end
...
if doStatFCAll == 1
save([AnimalGroupDir 'statFCAll_' toiWinName normFix '_' nAnimal '.mat'],'statFCAll','-v7.3');
end
if doStatFCPx == 1
save([AnimalGroupDir 'statFCPx' normFix '_' nAnimal '.mat'],'statFCPx','-v7.3');
end
end
%% Calculate correlation
if doCorrFCAll == 1 && ~exist('statFCAll'); load([AnimalGroupDir 'statFCAll_' toiWinName normFix '_' nAnimal '.mat']);end
if doCorrFCPx == 1 && ~exist('statFCPx'); load([AnimalGroupDir 'statFCPx' normFix '_' nAnimal '.mat']);end
...
for iRegion = 1:region.N
    ...
    for iOpto = 1:numOptos
        optoName = optoNames{optoIDs(iOpto)};
        behav = statBehavAccP7b(:,optoName);   
        if doCorrFCAll == 1
        %FClevelMask = ismember(statFCAll.(['Spec_' regionName]).(optoName).SessionType(:,7), {'a','b'});
        ephys = statFCAll.(['Spec_' regionName]).(optoName)(:,'Value');
        end
        if doCorrFCPx == 1
        %FClevelMask = ismember(statFCPx.(['Spec_' regionName]).(optoName).SessionType(:,7), {'a','b'});
        ephysPx = statFCPx.(['Spec_' regionName]).(optoName)(:,'Value');
        end
        if ~strcmp(optoName,'Sham') % prepare for contrast correlation
            behavS = statBehavAccP7b(:,'Sham');
            behavCon = behav{:,1} - behavS{:,1};
            if doCorrFCAll == 1
            ephysCon = statFCAll.(['Spec_' regionName]).([optoName '_Sham'])(:,'Value');
            end
            if doCorrFCPx == 1
            ephysPxCon = statFCPx.(['Spec_' regionName]).([optoName '_Sham'])(:,'Value');
            end
        end
        if doCorrFCPx == 1
        for iF = 1:nPx
            fprintf(repmat('\b', 1, lastsize));
            lastsize = fprintf(['Processing  Spec~AccP ' regionName ' ' optoName ' ' num2str(nPx) ' freq x t: ' num2str(iF)]);
...
for iPair = 1:numRegionPairs
    ...
    for iOpto = 1:numOptos
        ...
        ephys = statFCAll.(['PLV_' regionPair_Name]).(optoName)(:,'Value');
        end
        if doCorrFCPx == 1
        %FClevelMask = ismember(statFCPx.(['PLV_' regionPair_Name]).(optoName).SessionType(:,7), {'a','b'});
        ephysPx = statFCPx.(['PLV_' regionPair_Name]).(optoName)(:,'Value');
        end
        if ~strcmp(optoName,'Sham') % get contrast correlation
            behavS = statBehavAll.accP(levelMask & HMMask,'Sham');
            behavCon = behav{:,1} - behavS{:,1};
            if doCorrFCAll == 1
            ephysCon = statFCAll.(['PLV_' regionPair_Name]).([optoName '_Sham'])(:,'Value');
            ephysConD = statFCAll.(['PLV_' regionPair_Name]).([optoName '_ShamD'])(:,'Value');
            end
            if doCorrFCPx == 1
            ephysPxCon = statFCPx.(['PLV_' regionPair_Name]).([optoName '_Sham'])(:,'Value');
            ephysPxConD = statFCPx.(['PLV_' regionPair_Name]).([optoName '_ShamD'])(:,'Value');
            end
        end
        if doCorrFCPx == 1
        for iF = 1:nPx
            fprintf(repmat('\b', 1, lastsize));
            lastsize = fprintf(['Processing PLV~AccP ' regionPair_Name ' ' optoName ' ' num2str(nPx) ' freq x t: ' num2str(iF)]);
...
if doCorrFCAll == 1
save([AnimalGroupDir 'corrFCAll_' toiWinName normFix '_' nAnimal '.mat'],'corrFCAll');
end
if doCorrFCPx == 1
save([AnimalGroupDir 'corrFCPx' normFix '_' nAnimal '.mat'],'corrFCPx');
end

%% Calculate variance of each pixel or each frequency across sessions (opto reduces variance)
if doVar == 1
    if ~exist('statFCAll'); load([AnimalGroupDir 'statFCAll_' toiWinName normFix '_' nAnimal '.mat']);end
    if ~exist('statFCPx'); load([AnimalGroupDir 'statFCPx' normFix '_' nAnimal '.mat']);end
lastsize = 0;
for iRegion = 1:region.N
    regionName = region.Names{iRegion};
    for iOpto = 1:numOptos
...
%         FClevelMask = ismember(statFCAll.(['Spec_' regionName]).(optoName).SessionType(:,7), {'a','b'});
        ...
        ephys = statFCAll.(['Spec_' regionName]).(optoName)(:,'Value');
        varF.(['Spec_' regionName]).(optoName) = NaN(1,nFoi);
        for iF = 1:nFoi
            thisFreq = ephys{:,1}(:,iF);
        ...
        ephysPx = statFCPx.(['Spec_' regionName]).(optoName)(:,'Value');
        varPx.(['Spec_' regionName]).(optoName) = NaN(1,nPx);
        for iF = 1:nPx
            fprintf(repmat('\b', 1, lastsize));
            lastsize = fprintf(['Processing Var ' regionName ' ' optoName ' ' num2str(nPx) ' freq x t: ' num2str(iF)]);
            thisFreq = ephysPx{:,1}(:,iF);
            varPx.(['Spec_' regionName]).(optoName)(iF) = var(thisFreq);
        end
    end
end
save([AnimalGroupDir 'varFCAll_' toiWinName normFix '_' nAnimal '.mat'],'varF');
save([AnimalGroupDir 'varFCPx' normFix '_' nAnimal '.mat'],'varPx');
end

%% plot variance for all pixel
if doPlotVar == 1
    if ~exist('statFCAll'); load([AnimalGroupDir 'varFCAll_' toiWinName normFix '_' nAnimal '.mat']);end
    if ~exist('statFCPx'); load([AnimalGroupDir 'varFCPx' normFix '_' nAnimal '.mat']);end
...
for iRegion = 1:region.N
    ...
    for iOpto = 1:numOptos
        ...
        imagesc(t, 1:nFoi, reshape(varPx.(['Spec_' regionName]).(optoName)',size(t,2),nFoi)') % reshape to spectrogram dimension and order
        ...
        ylabel(cl,{['n=' num2str(nSess) ' ' nAnimal ' Power' normF ' var'];[regionName ': ' optoName]},'FontSize',12)
        else
        ...
        subplot(numOptos,region.N,iRegion+(iOpto-1)*region.N)
        scatter(1:nFoi,varF.(['Spec_' regionName]).(optoName),5,'filled')
        set(gca,'XTick',tickLoc,'XTickLabel',tickLabel)
        if iRegion ==1 && iOpto == 1
        title({['n=' num2str(nSess) ' ' nAnimal ' Power' normF ' var'];[regionName ': ' optoName]})
        else
...
savefig(fig1, [AnimalGroupDir 'var_pow' normF '_allPx_' nAnimal '.fig'],'compact');
saveas(fig1, [AnimalGroupDir 'var_pow' normF '_allPx_' nAnimal '.png']);
savefig(fig2, [AnimalGroupDir 'var_pow' normF '_allFreq_' nAnimal '.fig'],'compact');
saveas(fig2, [AnimalGroupDir 'var_pow' normF '_allFreq_' nAnimal '.png']);
end
...
for iRegion = 1:region.N
    ...
    for iOpto = 1:numOptos
        ...
        subplot(numOptos,region.N,iRegion+(iOpto-1)*region.N)
        imagesc(t, 1:nFoi, reshape(r',size(t,2),nFoi)') % reshape to spectrogram dimension and order
        xlabel(xLabel); ylabel(yLabel);% title('PLV')
        set(gca,'YDir','normal','TickDir','out','YTick',tickLoc,'YTickLabel',tickLabel)
        ylim([tickLoc(1) tickLoc(end)-10]);set(gca,'YDir','normal','TickDir','out','YTick',tickLoc,'YTickLabel',tickLabel)
        cl = colorbar('northoutside'); 
        if iRegion ==1 && iOpto == 1
        ylabel(cl,{['n=' num2str(nSess) ' ' nAnimal ' Power' normF '~AccP corr'];[regionName ': ' optoName]},'FontSize',12)
        else
        ylabel(cl,[regionName ': ' optoName],'FontSize',12)            
        end
...
savefig(fig, [AnimalGroupDir 'corr_pow' normF '~accP_allPx_' nAnimal '.fig'],'compact');
saveas(fig, [AnimalGroupDir 'corr_pow' normF '~accP_allPx_' nAnimal '.png']);
...
for iRegion = 1:region.N
    ...
    for iOpto = 1:numOptos
        ...
        c = corrFCPx.SpecAccP.(regionName){iOpto*2,2}; % p-value
        subplot(numOptos,region.N,iRegion+(iOpto-1)*region.N)
        imagesc(t, 1:nFoi, reshape(c',size(t,2),nFoi)') % reshape to spectrogram dimension and order
        xlabel(xLabel); ylabel(yLabel);% title('PLV')
        set(gca,'YDir','normal','TickDir','out','YTick',tickLoc,'YTickLabel',tickLabel)
        ylim([tickLoc(1) tickLoc(end)-10]);set(gca,'YDir','normal','TickDir','out','YTick',tickLoc,'YTickLabel',tickLabel)
        cl = colorbar('northoutside'); 
        if iRegion ==1 && iOpto == 1
        ylabel(cl,{['n=' num2str(nSess) ' ' nAnimal ' Power' normF '~AccP p-value'];[regionName ': ' optoName]},'FontSize',12)
        else
        ylabel(cl,[regionName ': ' optoName],'FontSize',12)            
        end
...
savefig(fig, [AnimalGroupDir 'corrP_pow' normF '~accP_allPx_' nAnimal '.fig'],'compact');
saveas(fig, [AnimalGroupDir 'corrP_pow' normF '~accP_allPx_' nAnimal '.png']);
...
for iPair = 1:numRegionPairs
    ...
    for iOpto = 1:numOptos
        ...
        cl = colorbar('northoutside'); 
        if iPair ==1 && iOpto == 1
        ylabel(cl,{['n=' num2str(nSess) ' ' nAnimal ' PLV~AccP corr'];[regionPairName ': ' optoName]},'FontSize',12)
        else
...
savefig(fig, [AnimalGroupDir 'corr_PLV~accP_allPx_' nAnimal '.fig'],'compact');
saveas(fig, [AnimalGroupDir 'corr_PLV~accP_allPx_' nAnimal '.png']);
...
for iPair = 1:numRegionPairs
    ...
    for iOpto = 1:numOptos
        ...
        cl = colorbar('northoutside'); 
        if iPair ==1 && iOpto == 1
        ylabel(cl,{['n=' num2str(nSess) ' ' nAnimal ' PLV~AccP p-value'];[regionPairName ': ' optoName]},'FontSize',12)
        else
        ylabel(cl,[regionPairName ': ' optoName],'FontSize',12)            
        end
        colormap(flipud(awesomeMap)); caxis([0,0.1]);%colormap %
...
savefig(fig, [AnimalGroupDir 'corrP_PLV~accP_allPx_' nAnimal '.fig'],'compact');
saveas(fig, [AnimalGroupDir 'corrP_PLV~accP_allPx_' nAnimal '.png']);
...
for iRegion = 1:region.N
    ...
    for iOpto = 1:numOptos-1
        ...
        cl = colorbar('northoutside'); 
        if iRegion ==1 && iOpto == 1
        ylabel(cl,{['n=' num2str(nSess) ' ' nAnimal ' Power' normF 'Diff~AccP corr'];[regionName ': ' optoName '-Sham']},'FontSize',12)
        else
...
savefig(fig, [AnimalGroupDir 'corr_pow' normF 'Diff~accPDiff_allPx_' nAnimal '.fig'],'compact');
saveas(fig, [AnimalGroupDir 'corr_pow' normF 'Diff~accPDiff_allPx_' nAnimal '.png']);
...
for iRegion = 1:region.N
    ...
    for iOpto = 1:numOptos-1
        ...
        cl = colorbar('northoutside'); 
        if iRegion ==1 && iOpto == 1
        ylabel(cl,{['n=' num2str(nSess) ' ' nAnimal ' Power' normF 'Diff' normF '~AccP p-value'];[regionName ': ' optoName '-Sham']},'FontSize',12)
        else
        ylabel(cl,[regionName ': ' optoName '-Sham'],'FontSize',12)            
        end
        colormap(flipud(awesomeMap)); caxis([0,0.1]);%colormap
...
savefig(fig, [AnimalGroupDir 'corrP_pow' normF 'Diff~accPDiff_allPx_' nAnimal '.fig'],'compact');
saveas(fig, [AnimalGroupDir 'corrP_pow' normF 'Diff~accPDiff_allPx_' nAnimal '.png']);
...
for iPair = 1:numRegionPairs
    ...
    for iOpto = 1:numOptos-1
        ...
        cl = colorbar('northoutside'); 
        if iPair ==1 && iOpto == 1
        ylabel(cl,{['n=' num2str(nSess) ' ' nAnimal ' PLVDiff~AccP corr'];[regionPairName ': ' optoName]},'FontSize',12)
        else
...
savefig(fig, [AnimalGroupDir 'corr_PLVDiff~accPDiff_allPx_' nAnimal '.fig'],'compact');
saveas(fig, [AnimalGroupDir 'corr_PLVDiff~accPDiff_allPx_' nAnimal '.png']);
...
for iPair = 1:numRegionPairs
    ...
    for iOpto = 1:numOptos-1
        ...
        cl = colorbar('northoutside'); 
        if iPair ==1 && iOpto == 1
        ylabel(cl,{['n=' num2str(nSess) ' ' nAnimal ' PLVDiff~AccP p-value'];[regionPairName ': ' optoName]},'FontSize',12)
        else
        ylabel(cl,[regionPairName ': ' optoName '-Sham'],'FontSize',12)            
        end
        colormap(flipud(awesomeMap)); caxis([0,0.1]);%colormap
...
savefig(fig, [AnimalGroupDir 'corrP_PLVDiff~accPDiff_allPx_' nAnimal '.fig'],'compact');
saveas(fig, [AnimalGroupDir 'corrP_PLVDiff~accPDiff_allPx_' nAnimal '.png']);
...
for iRegion = 1:region.N
    ...
    for iOpto = 1:numOptos
        ...
        sz = 5;
        subplot(numOptos,region.N,iRegion+(iOpto-1)*region.N)
        scatter(1:nFoi,r,sz,c,'filled')
        set(gca,'XTick',tickLoc,'XTickLabel',tickLabel)
        if iRegion == 1
            title({['n=' num2str(nSess) ' ' nAnimal ' ' toiWinName ' Mn'];[regionName ': ' optoName]});
        else
            title([regionName ': ' optoName]);
        end
        xlabel('Freq [Hz]'); %ylim([-0.3,0.15]); 
...
savefig(fig, [AnimalGroupDir 'corr_pow' normF '~accP_' toiWinName '_allFreq_' nAnimal '.fig'],'compact');
saveas(fig, [AnimalGroupDir 'corr_pow' normF '~accP_' toiWinName '_allFreq_' nAnimal '.png']);
...
for iPair = 1:numRegionPairs
    ...
    for iOpto = 1:numOptos
        ...
        set(gca,'XTick',tickLoc,'XTickLabel',tickLabel)
        if iRegion == 1
            title({['n=' num2str(nSess) ' ' nAnimal ' ' toiWinName ' Mn'];[regionPairName ': ' optoName]});
        else
            title([regionPairName ': ' optoName]);
        end
        xlabel('Freq [Hz]'); %ylim([-0.3,0.15]); ylabel(['PLV change ~ AccP corr coef']);
        ylim([-0.3,0.3]);
        colormap(flipud(jet)); caxis([0,0.1]);
        cl=colorbar();set(cl, 'YTick',[0,0.05,0.1]); title(cl,'p');
    end
end
savefig(fig, [AnimalGroupDir 'corr_PLV~accP_' toiWinName '_allFreq_' nAnimal '.fig'],'compact');
saveas(fig, [AnimalGroupDir 'corr_PLV~accP_' toiWinName '_allFreq_' nAnimal '.png']);
...
for iRegion = 1:region.N
    ...
    for iOpto = 1:numOptos-1
        ...
        set(gca,'XTick',tickLoc,'XTickLabel',tickLabel)
        if iRegion == 1
            title({['n=' num2str(nSess) ' ' nAnimal ' ' toiWinName ' Mn'];[regionName ': ' optoName '-Sham']});
        else
            title([regionName ': ' optoName '-Sham']);
        end
        xlabel('Freq [Hz]'); %ylim([-0.3,0.15]); ylabel(['Power[dB]Diff~AccPDiff corr coef']);
        ylim([-0.3,0.3]);
        colormap(flipud(jet)); caxis([0,0.1]);
        cl=colorbar();set(cl, 'YTick',[0,0.05,0.1]); title(cl,'p');
    end
end
savefig(fig, [AnimalGroupDir 'corr_pow' normF 'Diff~accPDiff_' toiWinName '_allFreq_' nAnimal '.fig'],'compact');
saveas(fig, [AnimalGroupDir 'corr_pow' normF 'Diff~accPDiff_' toiWinName '_allFreq_' nAnimal '.png']);
...
for iPair = 1:numRegionPairs
    ...
    for iOpto = 1:numOptos-1
        ...
        set(gca,'XTick',tickLoc,'XTickLabel',tickLabel)
        if iRegion == 1
            title({['n=' num2str(nSess) ' ' nAnimal ' ' toiWinName ' Mn'];[regionPairName ': ' optoName '-Sham']});
        else
            title([regionPairName ': ' optoName '-Sham']);
        end
        xlabel('Freq [Hz]'); %ylim([-0.3,0.15]); ylabel(['Power[dB]Diff~AccPDiff corr coef']);
...
savefig(fig, [AnimalGroupDir 'corr_PLVDiff~accPDiff_' toiWinName '_allFreq_' nAnimal '.fig'],'compact');
saveas(fig, [AnimalGroupDir 'corr_PLVDiff~accPDiff_' toiWinName '_allFreq_' nAnimal '.png']);
...
savefig(fig, [AnimalGroupDir 'corr_PLVRatio~accPDiff_' toiWinName '_allFreq_' nAnimal '.fig'],'compact');
saveas(fig, [AnimalGroupDir 'corr_PLVRatio~accPDiff_' toiWinName '_allFreq_' nAnimal '.png']);
end