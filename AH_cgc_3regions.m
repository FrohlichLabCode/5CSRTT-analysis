function cgc = AH_cgc_3regions(xser,yser,cser,regionXname,regionYname,regionCname,condName,alignHitName,...
                    fs,event,twin, baseTwin, chn1,chn2,chn3, saveRootPath)
...
try
    ...
    xsig = AH_interp(xsig); ysig = AH_interp(ysig); csig = AH_interp(csig);  
    xser = AH_interp(xser); yser = AH_interp(yser); cser = AH_interp(cser);

cgc = sub_grangerCausality(cgc,xser,yser,cser,isnan(xsig),isnan(ysig),isnan(csig),event,fs,twin,foi);
catch
end
...
function cgc = sub_grangerCausality(cgc,xsig,ysig,csig,xnan,ynan,cnan,event,fs,twin,foi)
...
xsig = double(xsig); ysig = double(ysig); csig = double(csig); 
