function regionPair_FunConn_V4(cluster, skipRec, MedianorPCA, recName,trialIDs,...
    evtTimes,twins,baseTwins, condNames, condIDs, regionPairs, regionNames, alignHitName,...
...
% This V3 version added different opto conditions, corresponding to level7*
% AH 20190325 added parfor on channels, try to combine channel1 and
% channel2, not successful, needs to change all structs;
% added prealocation of matrix for speed
% AH 20190410 added input "cluster" to switch between parfor and for
% V4 works for eeglab preprocessing pipeline, changed condName and
% alignHitName, added subsampTrialN to control for total number of trials
...
%     end
    ...
    cd([saveAnalysisDir]);
    % save means
    save(['FC_' alignHitName condName '_MntriMnchn.mat'],'tvec','foi','subTrialIDs','avgXSpec','avgYSpec','avgXNormed', 'avgYNormed','avgPLV','avgCoherency','avgImagZ','avgpsiNorm', '-v7.3');
    fprintf('\nDone saving mean');
    end
...
%     save(['psiAll_' condName '_' optoName '.mat'],'psiAll','psiFreq','psiNormAll','-v7.3');
    if doGC == 1
    try
    save(['GCAll_' alignHitName condName '.mat'],'tvecGC','subTrialIDs','GC_XtoY_All','GC_YtoX_All','avgGC_XtoY','avgGC_YtoX','-v7.3');
    catch
    end
    end
    ...
    cd([saveAnalysisDir]);    
    save(['FC_' alignHitName condName '_MdtriMdchn.mat'],'tvec','foi','subTrialIDs','avgXSpec','avgYSpec','avgXNormed', 'avgYNormed','avgPLV','avgCoherency','avgImagZ','avgpsiNorm', '-v7.3');
    if doGC == 1
    try
    save(['GC_' alignHitName condName '_MdtriMdchn.mat'],'tvecGC','subTrialIDs','avgGC_XtoY','avgGC_YtoX','-v7.3');
    catch
    end
    end
    fprintf('\nDone saving median ============================================\n')
